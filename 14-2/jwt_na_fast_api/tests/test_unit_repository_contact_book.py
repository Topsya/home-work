...
class TestContact(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_contact(self):
        body = ContactMoedels( name = 'John', surname = 'Street',  email = 'asdd@ddddd.net', fonenamber = '+380505555555', birthday = date(day=12, month=12, year=2000), id= self.user.id )
        
        result = await create_contact(body=body, db=self.session, user= User)
